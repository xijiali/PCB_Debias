...
from reid.trainers_partloss import Trainer_3stripes
from reid.evaluators import Evaluator,Evaluator_3stripes
...
#        model.load_state_dict(checkpoint['state_dict'])
    ...
    evaluator_6stripes=Evaluator_3stripes(model)
    ...
    # Trainer
    trainer = Trainer_3stripes(model, criterion, 0, 0, SMLoss_mode=0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            #print('Entangle results.')
            #R1,mAP=evaluator.evaluate(query_loader, gallery_loader, dataset.query, dataset.gallery)
            R1,mAP=evaluator_6stripes.evaluate(query_loader, gallery_loader, dataset.query, dataset.gallery)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'PCB_3stripes_Market'))
