...
##---------------------------stripe1----------------------------------------------#
    ...
    def forward(self, x,main_f=False,detach=False):
        ...
        main_f = F.avg_pool2d(x, x.size()[2:])
        main_f = main_f.view(main_f.size(0), -1)

        #print('main_f size is:{}'.format(main_f.size()))#[32,2048]
        bn_main_f = self.feat_bn(main_f)
        prob = self.classifier(bn_main_f)
...
#=======================FCN===============================#
        if self.FCN:
            ...
            sx = x.size(2)/6
            kx = x.size(2)-sx*5
            x = F.avg_pool2d(x,kernel_size=(int(kx),x.size(3)),stride=(int(sx),x.size(3)))   # H4 W8
#========================================================================#            
            ...
            if detach:
                x0 = grad_mul_const(x0, 0.0)
                x1 = grad_mul_const(x0, 0.0)
                x2 = grad_mul_const(x0, 0.0)
                x3 = grad_mul_const(x0, 0.0)
                x4 = grad_mul_const(x0, 0.0)
                x5 = grad_mul_const(x0, 0.0)
            ...
            #print('out0 size is:{}'.format(out0.size()))#[32,2048,6,1]


            return out0, (c0, c1, c2, c3, c4, c5),F.normalize(bn_main_f),prob
...
class ResNet_6stripe(nn.Module):
    __factory = {
        18: torchvision.models.resnet18,
        34: torchvision.models.resnet34,
        50: torchvision.models.resnet50,
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 num_features=0, norm=False, dropout=0, num_classes=0, FCN=False, radius=1., thresh=0.5):
        super(ResNet_6stripe, self).__init__()
        ...
        # Construct base (pretrained) resnet
        if depth not in ResNet_6stripe.__factory:
            raise KeyError("Unsupported depth:", depth)
        resnet = ResNet_6stripe.__factory[depth](pretrained=pretrained)
        self.base = resnet
        ...
        if self.FCN:
            ...
            ##---------------------------stripe1----------------------------------------------#
            self.instance0 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq0 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq0.weight, std=0.001)
            ##---------------------------stripe1----------------------------------------------#
            ##---------------------------stripe1----------------------------------------------#
            self.instance1 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq1 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq1.weight, std=0.001)
            ##---------------------------stripe1----------------------------------------------#
            ##---------------------------stripe1----------------------------------------------#
            self.instance2 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq2 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq2.weight, std=0.001)
            ##---------------------------stripe1----------------------------------------------#
            ##---------------------------stripe1----------------------------------------------#
            self.instance3 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq3 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq3.weight, std=0.001)
            ##---------------------------stripe1----------------------------------------------#
            ##---------------------------stripe1----------------------------------------------#
            self.instance4 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq4 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq4.weight, std=0.001)
            ##---------------------------stripe1----------------------------------------------#
            ##---------------------------stripe1----------------------------------------------#
            self.instance5 = MLP(input_dim=self.num_features,dimensions=[self.num_features,self.num_features,self.num_classes])
            self.nnq5 = nn.Linear(self.num_classes, self.num_classes, bias=False)
            init.normal_(self.nnq5.weight, std=0.001)
            self.drop = nn.Dropout(self.dropout)
...
class ResNet_3stripe(nn.Module):
    __factory = {
        18: torchvision.models.resnet18,
        34: torchvision.models.resnet34,
        50: torchvision.models.resnet50,
    ...
    def forward(self, x, main_f=False, detach=False):
        ...
        if self.FCN:
            ...
            print('out0 size is:{}'.format(out0.size()))#[32,2048,3,1]

            return out0, (c0, c1, c2), F.normalize(bn_main_f), prob
